...
impl<'a> StashWalker {
    ...
    pub fn walk<F, E>(&self, filter: &'a F, emitter: &'a E)
...
fn my_test() {
    ...
    let mut nswalker = StashWalker::new(&perl);

    let main_file = sv_extract_pv(perl.get_sv("0", 0)).unwrap();
    println!("$0 = {:?}", main_file);
    
    nswalker.walk(
        |cv| CvFILE(cv).map_or(false, |s| &s == main_file),
        |name: &String, cv: *const libperl_sys::cv| {
