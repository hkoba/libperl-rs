...
- macrogen の preprocessor が `PERLVAR` / `PERLVARI` / `PERLVARA` /
  `PERLVARIC` の呼び出しを観測して `PerlvarDict` に集める。
  ファイルパス指定不要 (`wrapper.h` → `perl.h` → `intrpvar.h` /
  `perlvars.h` を辿る間に自動収集)。`#ifdef` も preprocessor が正しく評価。
- 同じパス上に **Rust マクロ** の生成を追加する:
  - `PL_main_root!(my_perl)` のような **per-variable declarative macro**
    (1 引数で `*mut PerlInterpreter` ポインタを受け取る)
- **`Perl::pl_xxx()` メソッド形は emit しない** (libperl-sys 層は `Perl`
  型を知らないため。必要なら libperl-rs 側で hand-written wrapper を作る)。
- 個別マクロにすることで、**バージョン差を「定義の有無」で表現**できる
  (上述の通りターゲット固定なので結果的に cfg は不要)。
- threaded build + prefix `I` での展開形は `(*$my_perl).I<name>`、
  threaded build + prefix `G` または non-threaded build では
...
を想定していたが、**`macro_rules!` のハイジーン規則**により実現不可能と
判明:

- `macro_rules!` 本体に書いた `my_perl` 識別子は **マクロ定義サイトで
  名前解決**される。call-site のローカル変数 `my_perl` を捕捉できない。
- proc-macro なら可能だが、macrogen 出力 (`include!` で取り込む静的ファイル)
  と proc-macro の組み合わせは workflow 上の摩擦が大きい。
- C プリプロセッサ的な「自由捕捉」を Rust の `macro_rules!` で実現する
  方法は存在しない。

代替として **`PL_main_root!(my_perl)`** という 1 引数形を採用。**`my_perl`
変数名規約 (§3.8) は維持** され、ユーザは `let my_perl = perl.as_ptr();`
で導入してから macro を呼ぶ。`#[xs_sub]` (将来実装) では本体に `my_perl`
仮引数が居るので、そのまま `PL_main_root!(my_perl)` と書ける。

書き味の比較:

```rust
// 当初想定 (実現不可能):
let root = PL_main_root!();

// 実際 (採用):
let my_perl = perl.as_ptr();
let root = PL_main_root!(my_perl);
```

#### 3.3.2 cross-crate 利用が前提 (intra-crate ではハマる)

`#[macro_export]` macro を `include!()` で取り込む構成では、**同じ crate
内**から `crate::PL_main_root!()` や `use crate::PL_main_root;` で参照
すると `macro_expanded_macro_exports_accessed_by_absolute_paths` lint
で拒否される。これは既知の Rust 制約 (issue #52234)。

実用上、`PL_xxx!()` macro は **libperl-sys を依存先として持つ別 crate
(libperl-rs / libperl-proto0 / consumer applications)** から使われるので、
この制約は問題にならない。libperl-sys 内のテストは `tests/` ディレクトリ
(integration test) には書けず、libperl-rs 側に置く。
...
- `PL_xxx!(my_perl)` macro が **明示的に受け取る引数の名前は慣例として
  `my_perl`** (どんな名前の `*mut PerlInterpreter` でも渡せるが、
  揃えることで C の `aTHX_` と同じ語彙を保つ)
- `perl_call!(F(args...))` macro が threaded build で `F` の第 1 引数に
  渡す変数 → `my_perl` (これは call-site の自由捕捉。`PL_xxx!()` と異なり
  `perl_call!` は呼び出し全体をくるむ形なので自由捕捉できる、§3.6)
...
fn do_something(perl: &Perl) {
    ...
    let root = PL_main_root!(my_perl);   // 明示引数として渡す
    perl_call!(Perl_sv_setiv(sv, 42));   // perl_call! は call-site 自由捕捉
}
...
fn my_helper(sv: *mut SV) {
    ...
    // non-threaded build:
    //   fn my_helper(sv: *mut SV)
    let root = PL_main_root!(my_perl);   // どちらのモードでも書き換え不要
    perl_call!(Perl_sv_setiv(sv, 42));
}
```

non-threaded build でも `PL_main_root!(my_perl)` は引数を受け取り、
中で `let _: *mut PerlInterpreter = $my_perl;` で型チェックして捨てる。
**1 ソースで両モードビルド可能** という性質を維持。
